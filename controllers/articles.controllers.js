...
exports.getArticles = (req, res, next) => {
  const { topic, author, search, sort_by, order } = req.query;
  ...
  if (!topic && !author) {
    ...
    return checkExists("topics", "slug", topic)
      .then(() =>
        fetchArticles(topic, author, search, sort_by, order).then(
          (articles) => {
            res.status(200).send({ articles });
          }
        )
...
exports.patchArticleById = (req, res, next) => {
  ...
  return checkExists("articles", "article_id", article_id)
    .then(() =>
      updateArticleById(article_id, articleBody).then((article) => {
        res.status(200).send({ article });
      })
    )
    .catch(next);
...
exports.postCommentByArticleId = (req, res, next) => {
  ...
  return checkExists("articles", "article_id", article_id)
    .then(() =>
      checkExists("users", "username", username).then(() =>
